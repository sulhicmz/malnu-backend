...
class BaseController extends Controller
{
    ...
    protected function validationErrorResponse(array $errors)
    {
        return $this->errorResponse(
            ...
            config('error-codes.error_codes.VALIDATION.FAILED', 'VAL_001'),
            $errors,
            422
        );
    }
    ...
    protected function notFoundResponse(string $message = 'Resource not found')
    {
        return $this->errorResponse(
            $message,
            config('error-codes.error_codes.RESOURCE.NOT_FOUND', 'RES_001'),
            null,
            404
        );
    }
    ...
    protected function unauthorizedResponse(string $message = 'Unauthorized')
    {
        return $this->errorResponse(
            $message,
            config('error-codes.error_codes.AUTH.UNAUTHORIZED', 'AUTH_005'),
            null,
            401
        );
    }
    ...
    protected function forbiddenResponse(string $message = 'Forbidden')
    {
        return $this->errorResponse(
            $message,
            config('error-codes.error_codes.AUTH.FORBIDDEN', 'AUTH_006'),
            null,
            403
        );
    }
    ...
    protected function serverErrorResponse(string $message = 'Internal server error')
    {
        return $this->errorResponse(
            $message,
            config('error-codes.error_codes.SERVER.INTERNAL_ERROR', 'SRV_001'),
            null,
            500
        );
    }
